...
(function () {
    ...
    function registerEventHandler() {
        ...
        $('#checkBtn').on('click', function (){
            ...
            //校验文章模板不能为空
            if ($("#templateValue").val() == null || $("#templateValue").val().trim() == ""){
                layer.alert("文章模板不能为空，请输入标题！");
                return;
            }
        ...
        $('#pageAddOrEdit .wj-back, #pageAddOrEdit .article-cancel').on('click', function () {
            $('.wj-upload').show();
            $('.wj-page').hide();
            $('#pageList').show();
        });
            ...
            if ($('input[name="mainImg"]').length == picListSql.length){
                ...
                $.ajax({
                    ...
                    data: JSON.stringify({
                        ...
                        sortNum: $('#sortNum').val(),
                        templateId: parseInt($('#templateValue').val()),
                        picList: picList,
                        content: html,
                        deleteFlag: 0
                    }),
    ...
    function initSubject() {
        ...
        $.ajax({
            ...
            success:function(result){
                ...
                for (var m in list){
                    var option = document.createElement("option");  //创建option
                    option.setAttribute("value",parseInt(m)+1);
                    option.innerText = list[m];
